...
if METHOD == 'Random':
    ...
    for train_index, test_index in kf.split(X, Y):
        ...
        classifier = RandomForestClassifier(bootstrap=True, class_weight=None, criterion='gini',
                    max_depth=MAX_DEPTH, max_features=3, max_leaf_nodes=None,
        ...
        score_test += accuracy_score(Y_test, Y_pred_test, normalize = True)

        if accuracy_score(Y_test, Y_pred_test, normalize = True) > best_score:
            best_score = accuracy_score(Y_test, Y_pred_test, normalize = True)
            best_X = X_test
            best_Y = Y_test
            best_clf = classifier

        score_train += accuracy_score(Y_train, Y_pred_train, normalize = True )
        cm_tot += confusion_matrix(Y_test, Y_pred_test)
        acc_tot += accuracy_score(Y_test,Y_pred_test)
        #class_tot += classification_report(Y_test, Y_pred)

cm = cm_tot / N_SPLITS
accuracy = acc_tot /N_SPLITS
score_test = score_test/N_SPLITS
score_train = score_train/N_SPLITS
#-----------------------------------EVALUATION----------------------------------
title = 'RandomForest2'
