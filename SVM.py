...
prova = svm.LinearSVC(C=C)
prova.fit(X_train, Y_train)
f_importances(prova.coef_, features_names)

# we create an instance of SVM and fit out data.
models = (svm.SVC(kernel='linear', C=C, max_iter = 100000000),
          svm.LinearSVC(C=C, max_iter = 1000000),
          svm.SVC(kernel='rbf', gamma=GAMMA, C=C, max_iter = 1000000),
          svm.SVC(kernel='poly', degree=POLY_DEGREE, C=C, max_iter = 1000000))
...
for classifier, title in zip(models, titles):
    ...
    sn.heatmap(confusion_matrix(Y_test, Y_pred), annot=True, cmap='Blues')
